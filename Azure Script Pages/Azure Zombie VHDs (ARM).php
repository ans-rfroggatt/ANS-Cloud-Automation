...
if(!isset($_POST["submit"]))
{
    ?>
    <div class = "github">
    <a href="https://raw.githubusercontent.com/ans-rfroggatt/ANS-Cloud-Automation/master/Azure-Scripts/Azure-Zombie-VHDs-ARM.ps1">
...
elseif((isset($_POST["submit"])) && (!empty($_POST["username"])) && (!empty($_POST["password"])) && (!empty($_POST["subid"])) && (!empty($_POST["option"])))
{
    ...
    // If Option 1 pass in correct Parameters
    if($option == '1'){
        $query = shell_exec("powershell -command $psScriptPath -username '$username' -password '$password' -subId '$subid' -option '$option' -resourcegroup $resourcegroup -StorageAccountName $StorageAccountName");
    }
    
    // If Option 2 pass in correct Parameters
    if($option == '2'){
        $query = shell_exec("powershell -command $psScriptPath -username '$username' -password '$password' -subId '$subid' -option '$option' -searchstring $searchstring");
    }

    // If Option 3 pass in correct Parameters    
    if($option == '3'){
        $query = shell_exec("powershell -command $psScriptPath -username '$username' -password '$password' -subId '$subid' -option '$option'");    
    }
