...
<div class="page-banner">
  <div class="container text-center">
    <h1>Azure CloudHealth Agent Install</h1>      
...
if(!isset($_POST["submit"]))
{
    ...
    <div class="script-form" >
        <form class="script-form" name="testForm" id="testForm" action="Azure CHT Agent Install.php" method="post" />
	...
	Azure Subscription ID*: <input type="text" name="subid" id="subid" minlength="36" maxlength="36" value="<?php echo 'abcdefgh-1234-5678-abcd-123456789abc'; ?>" /><br />	
        Resource Group*: <input type="text" name="resourcegroup" id="resourcegroup" /><br />
        Windows API Key*: <input type="text" name="windowsapi" id="windowsapi" /><br />	
        Linux API Key*: <input type="text" name="linuxapi" id="lapikey" /><br />	
...
elseif((isset($_POST["submit"])) && (!empty($_POST["username"])) && (!empty($_POST["password"])) && (!empty($_POST["subid"])) && (!empty($_POST["resourcegroup"])) && (!empty($_POST["windowsapi"])) && (!empty($_POST["linuxapi"])))
{
    ...
    $windowsapi = $_POST["windowsapi"];
    $linuxapi = $_POST["linuxapi"];
         
    // Path to the PowerShell script. Remember double backslashes:
    $psScriptPath = "../Azure-Scripts/SetCustomScriptExtension.ps1";
 
    // Execute the PowerShell script, passing the parameters:
    $query = shell_exec("powershell -command $psScriptPath -username '$username' -password '$password' -subid '$subid' -resourcegroup '$resourcegroup' -windowsapi '$windowsapi' -linuxapi '$linuxapi'");
    ...
    $log = "../Azure-Script-Logs/" . $subid . "-CHTAgentLog.txt";
