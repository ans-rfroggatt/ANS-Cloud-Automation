...
function Test-ScriptFileInfo
{
    ...
    Process
    {
        ...
        if($ast)
        {
            ...
            $commentLines = $psscriptInfoComments.Text -split "`r`n" | % {$_.Trim()}
    ...
    if( $parts.Length -eq 5 )
    {
        ...
        if($options)
        {
            ...
            if($Scope -and $artfactType -eq $script:PSArtifactTypeScript)
            {
                ...
                {
                    Set-EnvironmentVariable -Name 'PATH' -Value "$currentPATHValue;$scriptDestination" -Target $envVariableTarget
                }
            }
